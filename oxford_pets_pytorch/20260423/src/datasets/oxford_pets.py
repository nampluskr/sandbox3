...
class OxfordPetsRegression(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        bbox_list = parse_xml2(sample["xml_path"])
        coords = torch.zeros((2, 4), dtype=torch.float32)

        if len(bbox_list) >= 2:
            coords[0] = torch.tensor(bbox_list[0], dtype=torch.float32)
            coords[1] = torch.tensor(bbox_list[1], dtype=torch.float32)
        else:
            coords = torch.zeros((2, 4), dtype=torch.float32)
...
class OxfordPetsDetection(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        boxes = tv_tensors.BoundingBoxes(bbox_list, format="XYXY", canvas_size=(h, w))
